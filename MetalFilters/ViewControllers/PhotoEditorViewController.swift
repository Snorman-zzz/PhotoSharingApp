...
class PhotoEditorViewController: UIViewController {
    ...
    private func setupNavigationButton() {
        let leftBarButton = UIBarButtonItem(title: "返回", style: .plain, target: self, action: #selector(cancelBarButtonTapped(_:)))
        let rightBarButton = UIBarButtonItem(title: "继续", style: .done, target: self, action: #selector(saveBarButtonTapped(_:)))
    ...
    @objc func saveBarButtonTapped(_ sender: Any) {
        guard let current = self.imageView.image,
            let uiImage = MTFilterManager.shared.generate(image: current) else {
            return
        }
